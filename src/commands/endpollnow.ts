...
import { endPoll } from "../services/pollService";

export const data = new SlashCommandBuilder()
  .setName("endpollnow")
  .setDescription(
    "Chiude subito una poll di loot ed estrae il vincitore, senza aspettare la scadenza (solo manager)"
  )
  .addStringOption((opt) =>
    opt
      .setName("poll")
      .setDescription("Quale poll chiudere subito")
      .setRequired(true)
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  await interaction.reply({
    content: `✅ Chiudo subito la poll #${poll.id} (**${poll.item_name}**) ed estraggo il vincitore...`,
    ephemeral: true,
  });
