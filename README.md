...
- **Ogni oggetto tracciato deve prima esistere come preset** (`/addpreset`). Questo non è un capriccio: garantisce che lo stesso oggetto venga sempre scritto in modo identico ovunque (inventario, poll), eliminando il rischio che typo diversi (es. "Mithril Ore" vs "Mithrilore" vs "Mythril Ore") vengano trattati come oggetti diversi e sballino i conteggi.
- `/addstock preset:"Mithril Ore" quantita:10` — un manager registra quanto è stato droppato, scegliendo il preset dal menu con autocomplete (non un campo di testo libero). L'icona viene presa automaticamente dal preset.
- Il pannello nel canale configurato con `/setinventorychannel` mostra un riquadro per ciascun oggetto, **con la sua icona**, nome, **quantità disponibile** e — se diversa da zero — **quanto è attualmente "in poll"** (riservato ma non ancora consegnato). Limite tecnico di Discord: massimo 10 oggetti diversi per messaggio: con più materiali di così, i successivi non vengono mostrati (ve lo segnala il bot stesso nel messaggio).
- Quando fai `/startpoll` **scegliendo un preset** (non il campo libero `oggetto`) per un oggetto che esiste in inventario, la quantità richiesta viene **spostata automaticamente da "disponibile" a "in poll"** — non puoi mettere in palio più di quanto hai davvero. Se la poll non ha vincitore (nessun iscritto, o tutti scesi sotto la soglia punti) o viene annullata con `/cancelpoll`, la quantità **torna disponibile**. Solo quando un manager conferma la consegna reagendo con 🎁 al messaggio del vincitore, la quantità **sparisce per sempre** dall'inventario.
...
- Il campo `oggetto` libero in `/startpoll` (per poll una tantum non tracciate) **non si collega mai** all'inventario, nemmeno se il nome coincide per caso con un oggetto esistente — solo il campo `preset` lo fa, proprio per eliminare ogni ambiguità sul nome.
